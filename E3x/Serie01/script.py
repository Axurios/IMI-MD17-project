...
def modify_train_script(folder,features, max_degree, num_iterations, num_basis_functions, cutoff,
                        num_train, num_valid, num_epochs, learning_rate, forces_weight, batch_size):
    with open(str(folder)+'/train.py', 'r') as file:
    ...
    with open(str(folder)+'/train.py', 'w') as file:
...
#L_num_basis_functions=[32,64]
L_num_basis_functions=[32]
L_cutoff=[3,4]
L_batch_size=[10,30,50]
L_num_valid=[k//5 for k in L_num_trains]
#L_learning_rate=[0.1,0.01,0.001]
L_learning_rate=[0.01]
#L_forces_weight=[0.01,0.1,1]
L_forces_weight=[1]
#L_features=[16,32,64]
L_features=[32,64]
max_degree=2

source_file_train="train.py"
source_file_run="run.py"
                ...
                for batch_size in L_batch_size:
                        for learning_rate in L_learning_rate:
                            for forces_weight in L_forces_weight:
                                ...
                                folder_path = "./"+str(features)+"_"+str(num_train)+"_"+str(num_basis_functions)+"_"+str(cutoff)+"_"+str(batch_size)+"_"+str(num_train // 5)+"_"+str(int(learning_rate*1000))+"_"+str(int(forces_weight*100))
                                print(folder_path)

                                if not os.path.exists(folder_path): 
                                    os.makedirs(folder_path)

                                shutil.copy(source_file_run, folder_path)
                                shutil.copy(source_file_train, folder_path)
                                shutil.copy(data, folder_path)
                                shutil.copy('jsub',folder_path)


                                modify_train_script(folder=folder_path,features=features, max_degree=max_degree, num_iterations=3, num_basis_functions=num_basis_functions, cutoff=3.0,
                        num_train=num_train, num_valid=num_train//5, num_epochs=300, learning_rate=learning_rate, forces_weight=forces_weight, batch_size=batch_size)
