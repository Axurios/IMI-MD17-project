...
def train_model(
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if epoch % 10 == 0:
            ...
            print(
                f"    loss [a.u.]             {train_loss : 8.3f} {valid_loss : 8.3f}"
            )
            print(
...
atomic_numbers = train_data["atomic_numbers"]
positions = train_data["positions"][0]
dummy_params = message_passing_model.init(
    key,
    atomic_numbers=atomic_numbers,
    positions=positions,
    dst_idx=dst_idx,
...
class MessagePassingCalculator(ase_calc.Calculator):
    ...
    def calculate(
        ...
        if isinstance(forces, jnp.ndarray):
            forces_np = np.array(
                jax.device_get(forces)
            )  # Ensure this is a 2D NumPy array, [num_atoms, 3]
        else:
...
for index_index, index in enumerate(random_array):
    ...
    with open(file_path, "a") as file:
        file.write(
            f"{features},{max_degree},{num_iterations},{num_basis_functions},{cutoff}\n"
        )

    # Write the hyperparameters
    with open(file_path, "a") as file:
        file.write(
            f"{num_train},{num_valid},{num_epochs},{learning_rate},{forces_weight},{batch_size},{index_index}\n"
        )
